...
describe('raw', () => {
  ...
  it('still throws on a non-2xx', async () => {
    ...
    const error = await air.raw.get('https://api.test/a').catch((e: unknown) => e)
...
describe('signals', () => {
  ...
  // Collects the signal handed to fetch on each call, which is the whole subject
  // of the tests below: one instance per request, or one shared by all of them.
  function signalSpy() {
